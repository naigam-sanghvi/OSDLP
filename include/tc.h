...
struct tc_mission_params {
	...
	uint16_t			max_data_size;		/* Maximum size of data portion of packet*/
...
__attribute__((weak))
int
rx_queue_enqueue(uint8_t *, uint16_t);
...
__attribute__((weak))
int
rx_queue_dequeue(uint8_t *, uint16_t);
...
__attribute__((weak))
int
rx_queue_enqueue_now(uint8_t *, uint8_t);
